...
require_once __DIR__ . '/../src/models/interfaces/IAlbum.php';
require_once __DIR__ . '/../src/models/Album.php';

require_once __DIR__ . '/../src/models/interfaces/IArtist.php';
require_once __DIR__ . '/../src/models/Artist.php';

require_once __DIR__ . '/../src/models/interfaces/IGenre.php';
require_once __DIR__ . '/../src/models/Genre.php';

require_once __DIR__ . '/../src/models/interfaces/IMediaType.php';
require_once __DIR__ . '/../src/models/MediaType.php';

require_once __DIR__ . '/../src/models/interfaces/IPlaylist.php';
require_once __DIR__ . '/../src/models/Playlist.php';

require_once __DIR__ . '/../src/models/interfaces/ITrack.php';
...
switch ($resource) {
    ...
    default:
        ...
        echo json_encode(['error' => 'Resource "' . htmlspecialchars($resource) . '" not found']);
        break;
}
    ...
    if ($method === 'GET') {
        ...
        if ($searchQuery !== null) {
            ...
            if ($artists === false || empty($artists)) {
                http_response_code(HttpStatusCodes::NOT_FOUND);
                echo json_encode(['error' => "No artists found matching '" . htmlspecialchars($searchQuery) . "'"]);
            } else {
                http_response_code(HttpStatusCodes::OK);
...
function handleAlbum(string $method, array $parts): void
{
    ...
    if ($method === 'PUT' && count($parts) === 1 && is_numeric($parts[0])) {
        ...
        $title = isset($input['title']) ? sanitizeText($input['title']) : null;
...
function handleTrack(string $method, array $parts): void
{
    ...
    if ($method === 'POST' && empty($parts)) {
        ...
        $input['composer'] = sanitizeText($input['composer']);
...
function handlePlaylist(string $method, array $parts): void
{
    ...
    if ($method === 'POST' && empty($parts)) {
        ...
        $newPlaylist = $playlistModel->create(sanitizeText($input['name']));
